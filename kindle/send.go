...
func Send(server string, port string, fromEmail string, password string, toEmail string, filePath string) {
	color.New(color.FgCyan).Add(color.Underline).Println("Sending to your kindle email...")
	...
	color.New(color.FgCyan).Add(color.Underline).Println("File sent successfully.")
}
